...
[CV](/assets/pdf/Ellie%5FY%5FCheng.pdf) 

[GitHub](https://github.com/ellieyhcheng/) 

Two recent lines of work have revealed revealed the benefits of marrying LLMs with programming environments, such as a Python REPL. Recursive Language Models (RLMs)[\[1\]](#zhang2025recursive) automates the work of managing long prompt contexts using by enabling LLMs to self-decompose context prompts.Also, our recent paper on **shared program state**[\[2\]](#cheng2025sharing) enables LLMs to modify the program state of a host program through a controlled interface. The common pattern of using an LLM for automated state management means programs using shared program state can easily use RLMs.
...
Figure of a RLM call from RLM blog post[\[3\]](#zhang2025rlm) 
...
We will explain how RLMs comes for free in programs with shared program state by an example program using our Nightjar library. The example is a program for executing the `trec_coarse` split of the OOLONG[\[4\]](#bertsch2025oolong) benchmark, which was used in the RLM work[\[1\]](#zhang2025recursive).
...
We evaluated the tasks with context window length of 131k, following the RLM paper[\[1\]](#zhang2025recursive). We also follow their experimental setup to use GPT-5 with medium reasoning effort as the root LLM and GPT-5-mini with medium reasoning as the sub-LLMs and to set recursion depth limit to 1\. We ran the benchmarks for N=3 runs and report the mean and standard deviation of metrics across runs.
